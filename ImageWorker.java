 ...
 * cache, running the work in a background thread and setting a place holder
 * image.
 */
public abstract class ImageWorker {
    ...
    protected OnSubString mOnSubString;

    private final static int IMAGEWORKER_TASK_COUNT = 5;
    private final static ExecutorService IMAGEWORKER_TASK_POOL = Executors.newFixedThreadPool(IMAGEWORKER_TASK_COUNT);
     ...
     * @param imageView
     *            The ImageView to bind the load task.
     */
    public void loadImage(Object data, ImageView imageView) {
        ...
        if (bitmap != null) {
            ...
            final BitmapOptionTask task = new BitmapOptionTask(imageView);
            final AsyncDrawable asyncDrawable = new AsyncDrawable(mContext.getResources(), mLoadingBitmap, task);
            imageView.setImageDrawable(asyncDrawable);
            task.executeOnExecutor(IMAGEWORKER_TASK_POOL, data);
        }
    }
    ...
    /**
     * Set place holder bitmap that shows when the the background thread is
     * running.
     * 
     * @param bitmap
     */
    ...
    public static void cancelWork(ImageView imageView) {
        final BitmapOptionTask bitmapWorkerTask = getBitmapWorkerTask(imageView);
     ...
     */
    private static BitmapOptionTask getBitmapWorkerTask(ImageView imageView) {
        if (imageView != null) {
     ...
     */
    private class BitmapOptionTask extends AsyncTask<Object, Void, Bitmap> {
        private Object data;
        private final WeakReference<ImageView> imageViewReference;

        public BitmapOptionTask(ImageView imageView) {
    ...
    private static class AsyncDrawable extends BitmapDrawable {
        private final WeakReference<BitmapOptionTask> bitmapWorkerTaskReference;

        public AsyncDrawable(Resources res, Bitmap bitmap, BitmapOptionTask bitmapWorkerTask) {
            super(res, bitmap);

            bitmapWorkerTaskReference = new WeakReference<BitmapOptionTask>(bitmapWorkerTask);
        }

        public BitmapOptionTask getBitmapWorkerTask() {
    ...
    private void setImageBitmap(ImageView imageView, Bitmap bitmap) {
        if (mFadeInBitmap) {
            // TransitionDrawable with a TransparentDrwabale and the final
            // bitmap
    ...
    public void setOnSubString(OnSubString mOnSubString) {
        this.mOnSubString = mOnSubString;
    }
